			...
			{ categoryData.length > 0 && categoryData.map((data) => (
				<div className={`${className} bg-gradient-to-r from-purpleP to-purple-400 dark:to-purple-500 relative`} key={data.id}>
					<div className={`${menuClass} bg-red-500 relative `}>
						...
						<div className={`w-[10rem] h-[5rem] py-2 duration-500 absolute top-8 right-0 bg-[rgba(0,0,0,0.4)] backdrop-blur-2xl rounded-md flex flex-col ${openMenu && menuId === data.id ? 'scale-[1]' : 'scale-[0]'}`} onClick={() => setOpenMenu(false)}>
					...
					<div className="text-right text-white" onClick={() => handleCategoryClick(data.id)}>
						<p className="text-lg"><i className="bx bx-task translate-y-[1px]"></i> {data.task_count}</p>						
					</div>
